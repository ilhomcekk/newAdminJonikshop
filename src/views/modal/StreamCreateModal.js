...
import { createReferralLink } from 'src/redux/actions/productActions'

const StreamCreateModal = ({ show, handleClose, data }) => {
  ...
  const handleSubmit = async () => {
    await createReferralLink(params)
      .then(({ data }) => {
  ...
  useEffect(() => {
    setParams((prev) => ({ ...prev, product: data }))
  }, [data])
    ...
    <div>
      <CModal visible={show} onClose={handleCloseState}>
        <CModalHeader onClose={handleCloseState}>
          <CModalTitle>Oqim yaratish</CModalTitle>
        </CModalHeader>
        <CModalBody>
          {!referralLink ? (
            <>
