...
const Tariffs = () => {
  ...
  const tariffs = useSelector((state) => state.tarif.tariffs)
  const _meta = useSelector((state) => state.tarif.pagination)
  ...
  useEffect(() => {
    ...
    dispatch(getTarif(newPagination))
  }, [metaNumber])
  ...
  return (
    <div className="card mb-4">
      <div className="card-header d-flex flex-wrap align-items-center">
        Тарифы
        <Link to={'/tariffs/create'} className="btn btn-success ms-auto text-white">
          Добавить
        </Link>
      </div>
        ...
        <table className="table table-hover table-striped">
          ...
          <tbody>
            {tariffs?.map((item, idx) => (
              <tr key={idx}>
                <th scope="row">
                  <Link to={`/tariffs/${item?._id}`} className="table-link"></Link>
                  {item?._id}
                </th>
                <td>{item?.name_ru}</td>
                <td>
                  {item?.price === '0' ? 'Договорная' : +item?.price?.toLocaleString('de-DE')}
                </td>
                ...
                <td className="table-icons-block">
                  <div className="table-icons">
                    <CButton
                      onClick={() => dispatch(postDeleteTarif(item._id))}
                      color="danger"
                      className="text-white"
                    >
                      <CIcon icon={cilTrash} />
                    </CButton>
                    <Link to={`/tariffs/update/${item?._id}`}>
