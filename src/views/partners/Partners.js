...
import { getPartners, postDeletePartners } from 'src/redux/actions/partnersActions'
...
const Partners = () => {
  ...
  const _meta = useSelector((state) => state.partners.pagination)
  ...
  useEffect(() => {
    ...
    dispatch(getPartners(newPagination))
  }, [metaNumber])
  ...
  return (
    <div className="card mb-4">
      <div className="card-header d-flex flex-wrap align-items-center">
        Партнеры
        <Link to={'/partners/create'} className="btn btn-success ms-auto text-white">
          Добавить
        </Link>
      </div>
        ...
        <table className="table table-hover table-striped">
          ...
          <tbody>
            {partners?.map((item, idx) => (
              <tr key={idx}>
                <th scope="row">
                  <Link to={`/partners/${item?._id}`} className="table-link"></Link>
                  {item?._id}
                </th>
                <td>
                  {' '}
                ...
                <td className="table-icons-block">
                  <div className="table-icons">
                    <CButton
                      onClick={() => dispatch(postDeletePartners(item?._id))}
                      color="danger"
                      className="text-white"
                    >
                      <CIcon icon={cilTrash} />
                    </CButton>
                    <Link to={`/partners/update/${item?._id}`}>
